...
This is not a stylistic preference. Each entry below fails on at least two of three counts: it
misrepresents something to a reader, it is retractable by the platform it targets, and it damages
the client when it is found. The list is short because it is meant to be remembered.
