...
> **Not entity-citation levers — a schema pile and an `llms.txt`.** Both reach an entity audit in the same shape, as "more markup, more citations", and both are already settled in [SETTLED-RULINGS.md](../../docs/loop/SETTLED-RULINGS.md). Refuse one on the substance, in the client's own terms — what the tactic does not do and what the page keeps instead; do not re-argue the ruling, and do not restate more of it than the refusal needs. The ruling IDs below are cited for whoever reads this skill and never for the report (root `CLAUDE.md`, the Reader Test, clause 1).
>
...
- [ ] No schema type is recommended in order to raise citation odds and no `llms.txt` is recommended — the page keeps one accurate primary type plus documented auxiliaries (settled ruling R2), and a type-pile or an llms.txt line already in the client's setup is refused in the report **on the substance, in the client's own terms** — what the tactic does not do and what the page keeps instead — because **the ruling IDs and this register path never appear in client prose** (root `CLAUDE.md`, the Reader Test, clause 1). The rulings behind the refusal are R2 and R1, [SETTLED-RULINGS.md](../../docs/loop/SETTLED-RULINGS.md), cited here for whoever reads this skill and never for the report
