...
`scripts/expectation-carrier-check.sh` (advisory, also not wired into the gate) finds the opposite problem: an eval expectation that grades a rule the skill never states, so the skill fails a test it was never told about. It greps each suite's quoted expectation phrases against the skill's own text. **Read its footer before trusting a clean line** — it sees uncarried *vocabulary* only, and misses an uncarried *behaviour* (measured coverage: **1 of the 3 known instances**, restated 2026-08-17). There are two distinct shapes it cannot see, and the second is worse than a miss. (i) A rule the skill never states in any words the grep can match. (ii) **A rule whose every phrase is present, so the tool scores it CARRIED, while the behaviour is absent where it acts** — measured on the `backlink-analyzer` disavow defect at `fba166c^`: the warning sentence, the sequence and the two-week window were all findable in the skill text, and the artefact still shipped without the warning. The tool asks *"is the phrase anywhere?"*, never *"is the rule implemented where it acts?"*, so shape (ii) **contributes to a clean line** rather than merely evading one. A grep-based extension for shape (ii) was built and measured on 2026-08-17 and **rejected on its numbers** — 11 candidates, 11 false positives, and it scores the known defective fence clean (that fence carried six comment lines already). The numbers are recorded in the script so nobody rebuilds it without beating them.

`scripts/fence-nesting-check.sh --bare-inner` (advisory, the fourth unwired check) reports a class the gated scan is structurally blind to: a **bare** fence closing a **tagged** container. The gated walk treats a bare fence only as a closer, so it sees ` ```html ` nested inside ` ```markdown ` and never sees the commoner authoring slip, where the author meant a nested block and CommonMark 4.5 reads a close — the container truncates and everything after it becomes a stray second block. Measured 2026-08-17 across 247 files: 307 bare-closes-tagged sites, 10 surviving three conditions, **10 of 10 hand-checked true, 0 false**. **That 10 is the pre-repair measurement and is quoted because the 10-of-10 precision is the finding; the current count is 6 sites in 5 files**, after four of the ten were repaired the same day by widening the outer fence to four backticks. The remaining six stand because they sat outside the repairing lane's file scope, not because they are disputed. `scripts/fence-nesting-check.sh`'s own header carries both figures and the six file:line sites, and is the accurate carrier — re-run it rather than quoting either number from here. The three conditions are container language is prose (a bare fence inside ` ```bash ` is unambiguously a close — that language has no fence syntax, so there is nothing to ask), the nesting reading balances (an odd run; an even one leaves the container open to EOF, which the gated scan already catches), and the escaped material carries markdown block syntax (the author's own evidence it belonged inside). **The finding names both readings and refuses to pick** — intent resolution is not encoded. Recall is not 1: a truncation whose escaped material is plain unformatted prose is invisible to it. It stays advisory until it exits 0 on a clean tree, because a leg wired in over known-true findings blocks every lane's push; that wiring condition is written into the script.
...
3. **A condition on the block travels inside the block** (added 2026-08-17). A paste-ready block is copied out on its own — into a CMS, a config file, a Search Console upload form — and the report frame around it stays behind on somebody's screen. So **every condition on that block's own safe use rides inside the fence, in that fence's own comment syntax**: the warning that makes it safe to run, the rollback that makes it reversible, the scope it must not exceed. This is not a new mechanism, it is the founding finding pointed at *conditions* instead of at values. It is a **second carriage point, not a replacement for the first** — the condition is also stated in the recommendation prose, and **one never stands in for the other**. The prose is read once, by whoever commissioned the work; the block outlives the report, gets forwarded, and is often executed by somebody who never read it, so a condition living only in the prose is missing at the moment of the irreversible action. Where the format has no comment syntax (JSON, JSON-LD), the condition rides a documented member and the fill instruction says so. A condition is **not** a label and does not reopen the closed list of three: labels say what kind of block this is, conditions say what it costs to run it.

Carriers: `build/schema-markup-generator/references/schema-templates.md` (12 JSON fences + the HTML array fence), `build/meta-tags-optimizer/references/meta-tag-code-templates.md` (the filled/skeleton split that solved this first). For clause 3: `monitor/backlink-analyzer/references/link-quality-rubric.md` §4 (the two carriage points, and why the disavow file's own comment block is part of the deliverable rather than an annotation on the example), `optimize/content-refresher/references/refresh-templates.md` (a caveat outside the copy does not travel with it — ledger F13).
...
2. **An operator block carries two labels, and neither stands in for the other** (amended 2026-08-17). An **in-fence label in that fence's own syntax** (`<!-- OPERATOR BLOCK … -->` for markdown and HTML) *and* a **visible line the reader sees rendered** — `**Next steps for your team** — *operator block; not part of the client report*`. Two, because **a comment alone is invisible in a rendered report, and a heading alone is lost when a model copies the fence and not the heading above it** — rank-tracker's wording, promoted from the one skill that already required both. The first half was ruled after both auditors' *Recommended Next Steps* blocks sat inside the client-read report fence carrying bare slugs, with the operator framing in prose *outside* it: worthless, for the Value Rule's founding reason. The second half after the mirror defect. `<!-- … -->` is HTML comment syntax, *displayed* in a skill file (it sits inside a fenced `markdown` block) and *rendered* in the delivered report, where it resolves to nothing — so the client saw the heading, the slugs and the score strings with no label at all. `SKELETON` and `ILLUSTRATIVE FILL` are deleted before delivery, so their invisibility is a missed tripwire in a failure case; `OPERATOR BLOCK` is the one label meant to **survive** into the report, and **it failed on every correctly-produced deliverable, not just the botched ones.** Where source and rendered output are the same surface (JSON, `robots.txt`, config) one label does both — the rule is two guarantees, not two lines for their own sake. Lifting the block out of the client fence entirely is still preferred, and still carries both. An operator block missing either label is family 8.
...
The test to apply, in two questions: **could a reader who copies only the fence tell this block is not for the client? Could a reader handed only the rendered report?** If either is no, it is not fixed.

Carriers: `build/seo-content-writer/references/anti-slop-ruleset.md` §6 family 8 and its "reader test, worked both ways" section; `references/inter-skill-handoff.md` §3.1 (the two-label rule, with the four fence-syntax label forms and which of them need a separate visible line). Provenance for every §6 rule lives in `build/seo-content-writer/references/anti-slop-provenance.md`, which is **not** an executor's read — see ledger F18.
