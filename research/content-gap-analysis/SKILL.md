...
Template. Three sites, one rule: change them together.
...
- [ ] Where Search Demand was proxied from competitor cluster depth, the row says Competitive Density is not independent evidence there — both are read off the same count, in opposite directions (Step 9) — and where a Quick Win Score is printed on that row, it says the two legs cancelled exactly and the screen read as Relevance + Effort
- [ ] The demand filter has a floor a single keyword can fail: >100/month on the volume path, or the named proxy's own floor in its own units, fixed and printed before any gap was scored (Step 4)
