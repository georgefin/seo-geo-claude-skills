...
**How this reconciles with the paragraph below, which is deliberately left byte-unchanged**: that paragraph still ends "state the 2023 restriction and stop there". Read it as scoped to the items it lists — the Search Console / Enhancements-filter / Rich-Results-Test / August-API set, which are still unsettled — **not** to the two dates above, which the owner check closed on 2026-08-19. It is left verbatim on purpose: it is the exact wording that check (f) in `scripts/validate-tracking.sh` accepts, and that check's own comment block (`:682`) says in terms that a miss must never be repaired by adding a marker, because every added marker widens the hole. Three rewrites of that paragraph were tried and each one either lost the binding or dropped the marker entirely; the honest repair was to stop editing the guarded line and put the reconciliation here instead. The remainder stays open at `docs/loop/r3-decision-brief.md`.
