...
- [ ] No unfilled slot survives in **finished** JSON-LD: no `[BRACKET]` token, no `_SKELETON` marker, no `TBD`/`XX`, no invented value — a value that cannot be sourced means the property is dropped and the gap is named in prose (missing-data rule, step 2). **Scope**: this bans an unfilled slot from markup presented as ready to paste. It does **not** apply to the bracket route of step 2 — a fill-in template the user asked for, which is *required* to keep its `"_SKELETON"` first member and its `[BRACKET]` slots precisely so it cannot be mistaken for ship-ready (step 2, and schema-templates.md "the one exception"). A skeleton block fails this checkbox only if it is offered as finished
