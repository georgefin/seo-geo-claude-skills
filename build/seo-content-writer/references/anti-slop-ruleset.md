...
| 9 | Engine-disposition claim in a Greek deliverable | «Η Google προτιμά σελίδες με σαφή ορισμό στην αρχή.» / «Το ChatGPT εξάγει τις πρώτες 40 λέξεις.» / «Οι μηχανές AI εμπιστεύονται περισσότερο τα .gr domains.» → the observational or conditional form: «Σε 12 από 20 ελέγχους που κάναμε στις 10 Αυγούστου, η απάντηση παρέθεσε σελίδα με ορισμό στην αρχή» · «δεν δημοσιεύει καμία μηχανή τα κριτήρια παράθεσης — αυτό είναι το μοντέλο εργασίας μας, όχι τεκμηριωμένη συμπεριφορά». This is **settled ruling R3 amendment 9a in Greek** — no primary source establishes what an engine cites, in either direction, so no deliverable may assert an engine disposition as fact. The rule already governed English; nothing carried it into the language half this library's deliverables are written in. **No script screens this for you.** The repo-text sweep that covers the English form scans this repository, and Greek deliverables are produced at run time and never live here — so on a Greek surface, this table is the only net. Provenance in `anti-slop-provenance.md` | Two-step screen, fixed literals only — **never a character range** (see the governing note). NET on the engine token: `Google` · `ChatGPT` · `Perplexity` · `Gemini` · `Copilot` · `\bAI\b` · `\bLLM` · `μηχαν` (covers «μηχανή», «μηχανές», «μηχανών») · `τεχνητή νοημοσύνη`. RANK the hits with the disposition stems, each unaccented where the stem allows so inflection does not escape: `προτιμ` · `εξάγ` · `εμπιστε` · `αναγνωρίζ` · `επιλέγ` · `αντλ` · `παραθέτ` · `κατανο` · `ανταμείβ` · `δίνει προτεραιότητα` · `θεωρ` · **`ταιριάζ` · `κανονικοποι` · `αγνο` · `κατατάσσ` · `φιλτράρ`**. Both steps case-insensitive **under `LC_ALL=C.UTF-8`**, set explicitly. **The last five were added 2026-08-17 to close a measured hole, and the measurement is the reason to trust the rest of the row.** A pilot lane writing a real Greek deliverable hit an engine claim whose verb was none of the original eleven, reported it, and the finding reproduced independently: on six constructed variants the NET caught **6 of 6** and the original RANK step **3 of 6** — it caught the founding shapes («προτιμά», «παραθέτει», «εμπιστεύονται») and missed all three *normalisation* claims («η Google **ταιριάζει** μόνη της τονισμένα και άτονα», «οι μηχανές **κανονικοποιούν** τους τόνους», «το ChatGPT **αγνοεί** τη διαφορά τόνου»). That is the half of the family a Greek keyword deliverable is most likely to write, because the normalisation claim is the one an SEO practitioner believes hardest. With the five stems added: **6 of 6**. Noise cost measured the same day over the live skill tree: `ταιριάζ` 1 file, the other four **0** — this widening is close to free. Stated limits: `επιλέγ`, `θεωρ` and `κατανο` are ordinary Greek that any SEO deliverable uses about its own author («επιλέγουμε λέξεις-κλειδιά»), and `αγνο` is a substring of «άγνωστος» and «αγνός», so all of them rank rather than net, and every ranked hit is read — the defect is the **subject**, not the verb. A copular Greek form («Η Google είναι αυτή που ανταμείβει…») and a claim split across two lines both escape a line-based screen |

| 10 | Guaranteed-outcome promise about a search or AI surface | «Εγγυόμαστε την πρώτη θέση…» · "We guarantee position 1" · "This work will get you cited by Perplexity" · "ensures every article ranks on page one" → the mechanism as a declared working model, plus a leading indicator with a measurement plan and a dated baseline: "an explicit room-size answer in the opening paragraph gives an assistant a directly liftable sentence — our working model, not documented engine behaviour; we will re-measure mention rate across 3 repeats on 40 prompts, monthly, from the 17 Aug baseline of 8 of 12 captures". **Distinct from family 9, and a deliverable can violate either alone**: family 9 bans asserting what an engine *does* («η Google προτιμά…»), family 10 bans promising a client what an engine *will do for them*. No engine publishes citation criteria, none guarantees determinism, and none offers a submission path that binds it — so the promise is a claim about a mechanism nobody has documented, not optimism. Ruled 2026-08-17 with the client mandate; carrier `references/prohibited-tactics.md` entry 9 and `references/ai-visibility-measurement.md` §7 | Two-step screen, both steps case-insensitive **under `LC_ALL=C.UTF-8`** (governing note below). NET on the commitment: `guarantee` (covers guarantees/guaranteed/guaranteeing) · `\bwill\b` · `you.ll` · `we commit` · `ensure` (covers ensures) · `εγγυ` (covers εγγύηση/εγγυημένη/εγγυόμαστε) · `θα σας` · `θα εμφαν` · `θα φέρ` · `δεσμευόμαστε`. RANK the hits on a search-outcome or engine token: `position [0-9]` · `#[0-9]` · `rank` · `top [0-9]` · `cited` · `citation` · `AI Overview` · `answers` · `recommended` · `ChatGPT` · `Perplexity` · `Gemini` · `Google` · `\bAI\b` · `πρώτη θέση` · `θέση` · `παραθέσ` · `αναφορ` · `απαντήσεις` · `ορατότητα`. **Measured 2026-08-17 per the amendment requirement below**: 10 constructed variants of its own defect (6 English, 4 Greek, spanning guarantee/will/ensure/εγγυ and four engines) — **10 of 10 caught**; 8 constructed negative controls — **0 survived**, including the two English forms that make a bare `guarantee` net unusable in e-commerce copy («Guaranteed next-day delivery», «The warranty guarantees the compressor for five years»), both correctly dropped by the RANK step. **The net's first form was narrower and measured 9 of 10.** It netted `we will` and `you will`; the escape was *"This work **will get you** cited by Perplexity"* — a promise whose subject is neither the agency nor the client but the work itself. Widened to bare `\bwill\b`, which only the RANK step makes usable. Stated limits: `\bwill\b` and `ensure` are ordinary English, so this is a **screen with a wide net and a narrow rank** — measured over the live skill tree the same day, 139 net hits reduce to 30 survivors, and the large majority of those are legitimate (a bad-example block quoting the defect, this library's own retractions of citation-likelihood language, and `Ensure you can rank` as a threshold instruction). A copular Greek form («Η πρώτη θέση είναι δεδομένη») and a promise split across two lines both escape, as they do in family 9. **First run found a real defect**, which is recorded because a family that has never caught anything is not evidence of a clean tree: `research/competitor-analysis/references/positioning-frameworks.md:32`, a **worked positioning example offered as a model to imitate**, reading *"is the content optimization platform that ensures every article ranks on page one"* — the recurring shape where a skill demonstrates the behaviour its own rules forbid. Fixed the same day |
...
default**, and no exit-status check catches it. **The PCRE escape hatch is not one** (measured
2026-08-17, same environment, while ruling on family 9): `grep -P '[\x{0370}-\x{03FF}]'` — the
form anyone reaches for once told to avoid `[α-ω]` — **exits 2 with "character code point value
in \x{} or \o{} is too large" under C, POSIX *and* `en_US.UTF-8`**, and works only under
`C.UTF-8`. The two range methods therefore fail in **complementary** locales, and there is no
locale in which both work. Worse for the reader of a script: `grep -rlP … | wc -l` prints `0`
and looks like an answer. The Greek census run for the family-9 ruling did exactly that and
reported **"0 files carry Greek" when 30 do**; it was caught only because the result was
cross-checked against a fixed-string search. **Match Greek with fixed literals. Never a range,
in either regex dialect.** Always set the locale,
and record the locale beside the count — and never assume `-i` did it; the licenser test
...
flagged («Κόστος μηδέν, δέσμευση καμία.», «Καμία χρέωση.»); **`ποτέ` matches inside «αποτέλεσμα»** — proven at the shell — but **not** inside the plural «αποτελέσματα», the genitive «αποτελέσματος» or the adverb «αποτελεσματικά»: the tonos moves off that ε in all three, so the substring is «ποτε» and the net misses it. This line claimed the plural until 2026-08-17 and was wrong about it (measured that day, `LC_ALL=C.UTF-8`, GNU grep 3.11). **Guard it the way the νίκη bullet guards the city, on the single preceding character: `grep -iP "(?<!α)ποτέ"`.** Measured 2026-08-17 on one blind run's Greek deliverable under `LC_ALL=C.UTF-8`: bare returned **10**, the guard **7**, and the 3 dropped were all «αποτέλεσμα» — every survivor a real n-word. On an 11-line constructed probe the guard kept **3 of 3** true positives, sentence-initial «Ποτέ» included, and dropped **3 of 3** word-internal hits («αποτέλεσμα» twice, «αποτέφρωση» once) where bare returned 6. **Do not reach for `(?<!απο)`: it is inert** — the match inside «αποτέλεσμα» is preceded by a single «α», never by three characters, so the lookbehind cannot fire and the negative always succeeds; measured returning the same 10 as bare, which is the failure mode that looks like a fix. Under `LC_ALL=POSIX` the guard still drops every false positive but `-i` stops case-folding Greek, so sentence-initial «Ποτέ» is missed (2 of 3 on the probe) — the locale rule above, not a fault in the guard. In a performance or analytics report «αποτέλεσμα» is unavoidable, so the **unguarded** net over-generates on any reporting deliverable; on the one deliverable measured that was 3 of 10 hits, a large minority rather than most. Questions and conditionals
...
- **«νίκη» for "win" — net on the noun, not the adjective.** «Γρήγορα κέρδη» / «Γρήγορη νίκη» for "quick wins" → «Άμεσες βελτιώσεις» / «Άμεσα οφέλη» / «Άμεσο όφελος» (or keep EN "quick wins"). Not «Γρήγορες διορθώσεις», which is natural Greek for *quick fixes* — see the protected list below. The ruling is about the metaphor, not the adjective: an entry netting the two-word phrase is escaped by any writer who changes the adjective («η πιο **γρήγορη** νίκη», «η **φθηνότερη** νίκη»), which is family 6's failure shape exactly, so the net is the noun `νίκη`, ranked with `γρήγορ` · `φθηνότερ` · `εύκολ`. **Guard — `νίκη` is a substring of «Θεσσαλονίκη».** Bare, the net fires on «Θεσσαλονίκη» / «Θεσσαλονίκης», a city that belongs in the delivery copy of half the Greek shops there are, and on given names built the same way («Βερονίκη», «Ανδρονίκη»). Screen the city out by negative context — `grep -P "(?<![Θθ]εσσαλο)νίκη"`, **both cases of the theta inside the lookbehind, and the bracket is load-bearing**. It returns «Η νίκη ήταν εύκολη», «η πιο γρήγορη νίκη», «η φθηνότερη νίκη» and «Γρήγορη νίκη: …» and drops the city in either case, under `LC_ALL=POSIX` *and* under `LC_ALL=C.UTF-8` (4 of 4 constructed true positives kept, 4 of 4 city lines dropped, both locales — measured 2026-08-17, GNU grep 3.11). **The capital-only form `(?<!Θεσσαλο)νίκη` shipped here from 2026-08-10 to 2026-08-17 and had a 100% false-positive rate on real Greek input**: keyword exports, ranking tables and hot-cache rows write the city lowercase, so every hit the guard returned was «θεσσαλονίκη». Measured 2026-08-17 over the two Greek `memory-management` fixtures and one blind run's five deliverables: the old form returned **1 + 1 + 3 = 5 hits, 5 of them false**; the case-fixed form returns **0 + 0 + 0**, under both locales. The 2026-08-10 measurement was not wrong about what it tested — it tested the capital — which is the constructed-variants requirement earning its place: a screen measured on one spelling of its own subject has been measured on half its input. It does not drop the given names, which stay a hand-check. Do **not** reach for `grep -w` instead: it does the right thing under `LC_ALL=C.UTF-8` and silently returns the Θεσσαλονίκη lines under `LC_ALL=POSIX`, where Greek letters are not word characters — measured, same session. Do not reach for a Greek bracket range either — but **for a reason this file stated wrongly until 2026-08-17**. `[α-ω]` aborts with `Invalid collation character` and exit 2 **under `C.UTF-8` only**. Under `POSIX`, which is this environment's default (`LANG` empty, `LC_CTYPE=POSIX`), it does **not** abort: measured at the shell 2026-08-17, exit **0**, matching all 3 Greek lines by byte. The loud failure is the one the range gives you in one locale; **the quiet one — exit 0 having matched by byte — is the default**, and no exit-status check catches it. The two-character-bracket workaround is not locale-safe either: `ιδανικ[ηήοό]` matched **3** under POSIX against **2** under `C.UTF-8` and `rg`, the extra being «ιδανικά», which shares a UTF-8 lead byte. Set `LC_ALL=C.UTF-8` explicitly, or use `rg`, and record the locale beside the count — an unrecorded locale makes a Greek result unreproducible. Where `-P` is unavailable, run the bare noun and hand-drop the place-name and given-name hits, saying they were dropped
...
- Regional/administrative forms and place names: «νομό Ηρακλείου», «Θεσσαλονίκη» /
  «Θεσσαλονίκης» — and **«θεσσαλονίκη» lowercase, the form a keyword export, a ranking table and a
  hot-cache row actually carry**, which is the form the guard above missed until 2026-08-17 (a
  city, not the quick-wins metaphor — see the `νίκη` guard above)
