...
- [ ] Every screen run over the finished deliverable (bracket tokens, `~~` tokens, a Greek regression net, a numeral census) is reported with its **exit status** as well as its output, and the status is read as **three states, not two**: `0` = ran and matched, so triage every hit · `1` = ran and matched nothing, which is what a clean screen looks like and is the status `grep` returns for it · `2` or more = did not run (invalid collation, bad regex, missing file, `127` command not found) and is **UNSCREENED**, never clean and never a pass. Neither fold is safe: calling exit 1 unscreened makes the guard cry wolf on every good run until somebody switches it off, and calling exit 2 clean lets a screen that never ran pass in silence. So the report carries the number and what was screened — "screened the deliverable, 214 lines, no match, exit 1" — because exit 1 on the wrong file reads exactly like exit 1 on the right one ([references/geo-score-arithmetic.md](./references/geo-score-arithmetic.md) §9, with the `set -e`-safe capture idiom)
- [ ] The client's sections stand alone. Strip every block labelled an operator block and re-read what is left: a cross-reference that no longer resolves — «βλ. σημείωμα ομάδας στο τέλος», "see the team note", "as noted in the handoff below" — is a FAIL. This is the inverse of an ID leak, so perfect labelling does not excuse it, and the labels themselves are not restated here: they are the closed list of three and the two-label rule in [inter-skill-handoff.md](../../references/inter-skill-handoff.md) §3.1 and root `CLAUDE.md`. **The positive half binds whether or not the deliverable carries an operator block**: what the client needs in order to act — that the structured-data code comes in a separate step, which figures are still missing — is stated in the client's own sections, in the client's words. This skill is a handoff *consumer*, not a producer (inter-skill-handoff.md §5.1/§5.2), so it emits no operator block by default; a deliverable that carries none passes on the positive half alone — that is the check satisfied, never the check skipped
...
- [GEO Score Arithmetic](./references/geo-score-arithmetic.md) - What every printed number is made of: the 1-10 scale as a ratio, what each factor counts, N/A handling, the lift, the pre-send recompute pass, and how to read a screen you ran over your own deliverable (exit 0 matched, exit 1 is the clean screen, exit 2 or more never ran)
