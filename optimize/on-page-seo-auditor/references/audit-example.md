...
2. **3 product images missing alt text** — Evidence: Sony WH-1000XM5, Bose QC Ultra, and Apple AirPods Max images have empty alt attributes (`alt=""`). Impact: lost Google Images ranking signals and weaker accessibility. Fix: set `alt="Sony WH-1000XM5 over-ear headphones"`, `alt="Bose QC Ultra over-ear headphones"` and `alt="Apple AirPods Max over-ear headphones"` — each drafted from the product the copy beside it reviews, so whoever can open the images confirms or corrects it before publishing. Confidence: Confirmed.
   - Owner: Content · Effort: S · Depends on: none
...
| 2 | Set the three drafted alt strings in Critical 2 on the Sony, Bose and Apple product images, confirming each against the photograph first | Content | All 8 `<img>` elements in the article body carry non-empty descriptive alt text, checked in view-source | Converts the alt-text criterion — returns 2 of Image Optimization's 10 | S | none | low — reversible; an alt describing something the photograph does not show is a false statement on the page, so confirm before publishing |
